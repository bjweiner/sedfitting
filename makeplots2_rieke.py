...
nsed_max = np.size(np.where(rieke_loglir < loglir_uplim))
...
####
# plot the Rieke template spectra
# change to obey loglir_uplim

plt.clf()
for i in range(nsed_touse):
    linestyle = 'k-'
    plt.plot(np.log10(sedrieke[i]['wave']), np.log10(sedrieke[i]['flux']), linestyle)
    dotstyle = 'ko'
...
fig = plt.ylabel('Rieke template flux, Jy')
plt.savefig(plotdir + 'rieke_templ_logflux.pdf')
...
# moved plotting of the best-9 to later

####
# fit best single SED to a series of Rieke models and plot each
...
for ii in range(len(iobs_array)):
    # iobs = 5
    iobs = iobs_array[ii]
    plotname = 'rieke_lir' + lir_name[ii] + '_onesed_fit.pdf'
    # iobs = 5
    testwave = filtwaves
    zobs = 0.003
    ...
    plt.clf()
    plt.plot(np.log10(sedrieke[iobs]['wave']), np.log10(sedrieke[iobs]['flux']), 'k-')
    ...
    plt.ylabel('log flux, Rieke template + 1-model fit')
    plt.savefig(plotdir + plotname)

# plt.savefig(plotdir + 'rieke_lir11_onesed_fit.pdf')
...
#plt.savefig(plotdir + 'rieke_lir11_onesed_fitv2.pdf')

##########
# monte carlo one model SED at a time to Rieke templates
...
plt.clf()
plt.plot(rieke_loglir[0:nsed], np.log10(mass_result_expect)+6, 'ko')
plt.errorbar(rieke_loglir[0:nsed], np.log10(mass_result_expect)+6, yerr=mass_result_expectrms/mass_result_expect/2.3026,fmt='ko')
fig = plt.xlabel('Rieke log IR luminosity, Lsun')
...
# itoplot = range(len(rieke_loglir))
# ntoplot = len(rieke_loglir) - 2
ntoplot = nsed_touse
itoplot = range(ntoplot)

plt.clf()
plt.plot(rieke_loglir[itoplot], mass_result_expectrms[itoplot]/mass_result_marginalrms[itoplot], 'ko')
plt.plot(rieke_loglir[itoplot], mass_result_expectrms[itoplot]/mass_result_marginalrms[itoplot], 'k-')
fig = plt.xlabel('Rieke log IR luminosity, Lsun')
...
##########
# fit a combination to a single Rieke SED and plot
...
for ii in range(len(iobs_array)):
    # iobs = 5
    iobs = iobs_array[ii]
    plotname = 'rieke_lir' + lir_name[ii] + '_combine_fit.pdf'
    testwave = filtwaves
    zobs = 0.003
    testflux = flux1[iobs,0:]
    testferr = 0.1 * testflux
    ...
    ax = plt.axis([0.3,3.0,0.0,4.0])
    plt.xlabel('log wavelength')
    plt.ylabel('log flux, Rieke template + combined fit')
    plt.savefig(plotdir + plotname)

# plt.savefig(plotdir + 'rieke_lir11.5_combine_fit.pdf')
...
# plot log Mdust estimate as fn of log LIR
plt.clf()
plt.plot(rieke_loglir[0:nsed], np.log10(mass_combine_mean)+6, 'ko')
plt.errorbar(rieke_loglir[0:nsed], np.log10(mass_combine_mean)+6, yerr=mass_combine_rms/mass_combine_mean/2.3026,fmt='ko')
...
plt.clf()
plt.subplot(1,1,1)
plt.plot(rieke_loglir[itoplot], np.log10(mass_result_expect[itoplot])+6, 'ro')
plt.plot(rieke_loglir[itoplot], np.log10(mass_result_expect[itoplot])+6, 'r-')
plt.errorbar(rieke_loglir[itoplot], np.log10(mass_result_expect[itoplot])+6, yerr=mass_result_expectrms[itoplot]/mass_result_expect[itoplot]/2.3026,fmt='ro')
fig = plt.xlabel('Rieke log IR luminosity, Lsun')
fig = plt.ylabel('log dust mass, Msun')
fig = plt.text(9.8,9.1,'one SED fit',color='red')
#fig = plt.axis([9.55,loglir_uplim,7.0,9.5])
#plt.subplot(2,1,2)
plt.plot(rieke_loglir[itoplot], np.log10(mass_combine_mean[itoplot])+6, 'bo')
plt.plot(rieke_loglir[itoplot], np.log10(mass_combine_mean[itoplot])+6, 'b-')
plt.errorbar(rieke_loglir[itoplot], np.log10(mass_combine_mean[itoplot])+6, yerr=mass_combine_rms[itoplot]/mass_combine_mean[itoplot]/2.3026,fmt='bo')
#fig = plt.xlabel('Rieke log IR luminosity, Lsun')
#fig = plt.ylabel('log combined dust mass, Msun')
...
plt.plot(rieke_loglir[itoplot], error_ratio, 'ko')
plt.plot(rieke_loglir[itoplot], error_ratio, 'k-')
fig = plt.xlabel('Rieke log IR luminosity, Lsun')
